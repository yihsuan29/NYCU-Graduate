...
class evaluation_model():
    def __init__(self, new = False, seed=30):
        ...
        for i in range(len(intermediate_samples)):
            denoise_plots.append(intermediate_samples[i][25])
        denoise_plots.append(images[25])
        
    
        self.generate_denoise(denoise_plots,new=self.new)
        self.generate_gridplot(images,new=self.new)
        self.generate_plot(images,new=self.new)
        
    def generate_denoise(self, images, new = False):
    ...
    def generate_gridplot(self, images, new = False):
    ...
    def generate_plot(self, images, new = False):     
        for i in range(len(images)):
