...
class Policy(nn.Module):
    ...
    def calculate_loss(self, gamma=0.999):
        ...
        for saved_action, R in zip(saved_actions, returns):
            R_B = R-saved_action.value
            policy_losses.append(-saved_action.log_prob * R_B)
    ...
    for i_episode in count(1):
        ...
        if ewma_reward > 120:
            if not os.path.isdir("./preTrained"):
                os.mkdir("./preTrained")
            torch.save(model.state_dict(), './preTrained/Baseline_LunarLander_{}.pth'.format(lr))
...
if __name__ == '__main__':
    ...
    #train(lr)
    test(f'Baseline_LunarLander_{lr}.pth')
